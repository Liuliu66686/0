...
addLayer("m", {
    ...
    upgrades: {
        ...
        41: {
            ...
            effect(){
                let eff = player.m.points.max(10).log(10).pow(2)
                return eff
            },
            effectDisplay(){return "x"+format(this.effect())},
    ...
    update(diff){
        ...
        if(player.m.muteP.gte(layers.m.angryMax())){
            player.m.muteP = zero;player.m.mutingT = player.m.mutingT.add(layers.m.muteTmaX()).min(layers.m.muteTmaX());player.m.angryTimes = player.m.angryTimes.add(1)
        }
    },
    microtabs:{
        mute1:{
            ...
            "刷屏":{
                ...
                content:[
                    "blank",["row",[["clickable",11],["clickable",12]]],"blank",["row",[["bar","sendL"],["bar","muteP"],["bar","muteT"]]],["display-text", function() {return "你被禁言了 "+format(player.m.angryTimes,0) + " 次<br>被禁言期间,打字速度/10!"}],"blank",["clickable",13]
                ],
                unlocked(){return hasUpgrade("m",11)||player.s.unlocked},
            },
        },
    ...
    microtabs:{
        stones:{
            "升级":{
                ...
                content:[
                    "blank","upgrades",["clickable",11],["display-text", function() {if(false) return "到达 10000 禁言点 解锁 "+quickColor("禁言石","#444444")}],
                ],
            },
        },
    },
